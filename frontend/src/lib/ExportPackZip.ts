...
async addEvent(type: string, payload: Record<string, unknown>): Promise<LedgerEntry> {
  ...
  private generateMetricsFromEvents(): string {
    ...
    const headers =
'sessionId,caseId,condition,initialBirads,finalBirads,aiBirads,aiModelName,aiModelVersion,aiProvider,changeOccurred,aiConsistentChange,addaDenominator,adda,preAiReadMs,postAiReadMs,totalReadMs,comprehensionItemId,comprehensionAnswer,comprehensionCorrect,comprehension_question_id,comprehension_answer,comprehension_correct,comprehension_response_ms';
