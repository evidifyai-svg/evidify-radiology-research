...
export const ResearchDemoFlow: React.FC = () => {
  ...
  const activeCase = state.currentCase ?? currentCase;

  // --- AI helper (compat: old aiResult vs newer case fields) ---
  const aiSuggestedBirads =
    (activeCase as any)?.aiResult?.birads ??
    (activeCase as any)?.aiBirads ??
    4;

  const aiSuggestedConfidence =
    (activeCase as any)?.aiResult?.confidence ??
    (activeCase as any)?.aiConfidence ??
    0.87;
