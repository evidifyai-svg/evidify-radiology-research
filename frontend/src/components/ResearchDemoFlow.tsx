...
const workloadMetrics = useWorkloadMetrics({
  ...
  onStatusChange: (previousStatus, newStatus, metrics) => {
    ...
    // Treat status change as a threshold crossing event for logging
    void eventLoggerRef.current
      .addEvent('WORKLOAD_THRESHOLD_CROSSED', {
        previousStatus,
        newStatus,
      ...
      .catch(() => {
        // Never break the flow if logging fails
      });
  },
});
...
setAiAgreementStreak(prev => prev + 1);
    ...
    workloadMetrics.recordCaseCompleted(completedCaseId);
