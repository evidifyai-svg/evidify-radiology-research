...
const generateExport = useCallback(async () => {
    ...
    for (const event of state.events) {
      await builder.addEvent(event.type, event.payload as Record<string, unknown>);
    }
    
    // Build pack
    const result = await builder.generateExportPackage();
    
    // Create a JSON blob for download (simplified - no zip for demo)
    const exportData = {
      manifest: result.manifest,
      events: builder.getEvents(),
      ledger: builder.getLedger(),
      metrics: builder.computeDerivedMetrics(),
      verifier: result.verifierOutput,
    };
    ...
    setExportUrl(url);
    setVerifierResult(result.verifierOutput.result);
  }, [state, timeOnCase]); 
  // ============================================================================
...
{/* Mammogram Viewer */}
          <div style={{ marginBottom: '24px' }}>
            ...
            showAI={false}
            onInteraction={(event) => console.log('Viewer:', event)}
          />
          </div>
 ...
 {exportUrl && (
            <button
              onClick={() => setShowPacketViewer(true)}
              style={{
                width: '100%',
                padding: '12px',
                fontSize: '16px',
      ...
      {showPacketViewer && exportPack && (
        <div style={{
          ...
          <div style={{
            ...
            <button
              onClick={() => setShowPacketViewer(false)}
              style={{
                position: 'absolute',
                top: 12,
                right: 12,
