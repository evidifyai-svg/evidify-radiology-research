...
const RELATIONSHIP_CATEGORIES = {
  family: { label: 'Family Member', icon: 'Family', biasRisk: 'high' },
  professional: { label: 'Professional', icon: 'Professional', biasRisk: 'low' },
  friend: { label: 'Friend/Acquaintance', icon: 'Peer', biasRisk: 'moderate' },
  legal: { label: 'Legal Party', icon: 'Legal', biasRisk: 'high' },
  other: { label: 'Other', icon: 'Other', biasRisk: 'unknown' },
};
    ...
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        ...
        <div className="flex gap-4 text-sm">
          ...
          {stats.highBiasSources > 0 && (
            <span className="px-2 py-1 bg-amber-200 text-amber-800 rounded">
              Warning: {stats.highBiasSources} high-bias source(s)
            </span>
          )}
        </div>
      </div>
      ...
      <div className="p-3 border-t bg-indigo-50">
        <p className="text-xs text-indigo-700">
          Note: High-bias sources may still provide valuable information when corroborated by other sources. 
